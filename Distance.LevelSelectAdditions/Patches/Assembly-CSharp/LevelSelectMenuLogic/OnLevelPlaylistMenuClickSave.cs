...
namespace Distance.LevelSelectAdditions.Patches
{
	...
	internal static class LevelSelectMenuLogic__OnLevelPlaylistMenuClickSave
	{
		...
		internal static bool Prefix(LevelSelectMenuLogic __instance, string absolutePath)
		{
			...
			Mod.Log.LogDebug($"Saving playlist file:  \"{curFile}\"");
			Mod.Log.LogDebug($"Saving playlist label: " + (curLabel != null ? $"\"{curLabel}\"" : "null"));
			Mod.Log.LogDebug($"Saving playlist Name:  " + (curName  != null ? $"\"{curName}\"" : "null"));
			...
			if (__instance.tempPlaylist_.Name_.IsEmptyPlaylistName())
			{
				...
				Mod.Log.LogDebug($"Saving playlist New Name:  " + (newName  != null ? $"\"{newName}\"" : "null"));
			}
